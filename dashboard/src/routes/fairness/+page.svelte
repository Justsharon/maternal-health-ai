...
<div class="mx-auto max-w-4xl px-6 py-12">
  ...
  {#if loading}
    <p class="text-slate-400">Loading fairness report…</p>
  {:else if error}
  ...
  {:else if report}
    <div class="space-y-6">
      <!-- Verdict card -->
<section class="rounded-2xl border border-slate-400 bg-slate-900 p-6">
  <div class="flex items-baseline justify-between gap-4 mb-4">
    <div>
      ...
      <p class="text-xs text-slate-400 mt-1">Named-group equalized-odds gap</p>
    </div>
    <div class="text-right text-sm">
      <p class="text-slate-400 mb-1">Gap</p>
      <p class="text-xl tabular-nums font-medium">
        {formatPct(report.named_group_gap)}
        <span class="text-xs text-slate-500">/ {formatPct(report.gap_threshold)} threshold</span>
      </p>
    </div>
  </div>
  <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-700 text-xs">
    <div>
      <p class="text-slate-500 mb-1">Full equalized-odds difference</p>
      <p class="tabular-nums text-slate-400">{formatPct(report.full_equalized_odds_difference)}</p>
    </div>
    <div>
      <p class="text-slate-500 mb-1">Auditor's overall flag</p>
      <p class="text-slate-400 uppercase">{report.overall_flag}</p>
    </div>
  </div>
</section>
            ...
            <li>
              <div class="flex items-baseline justify-between mb-1">
                ...
                <span class="text-sm tabular-nums text-slate-400">
                  {formatPct(g.recall)} <span class="text-xs text-slate-500">({g.n_total} positives)</span>
                </span>
              </div>
              <div class="h-1.5 rounded-full bg-slate-900-hover overflow-hidden">
...
<section class="rounded-2xl border border-slate-400 bg-slate-900 p-6">
  ...
  <ul class="space-y-3">
    {#each report.residual_groups as g}
      <li>
        <div class="flex items-baseline justify-between mb-1">
          <span class="text-sm text-slate-400">{formatGroup(g.group)}</span>
          <span class="text-sm tabular-nums text-slate-400">
            {formatPct(g.recall)} <span class="text-xs text-slate-500">(n={g.n_total})</span>
          </span>
        </div>
        <div class="h-1.5 rounded-full bg-slate-900-hover overflow-hidden">
          <div class="h-full bg-slate-500" style="width: {g.recall * 100}%"></div>
        </div>
      </li>
    {/each}
  </ul>
...
<section class="rounded-2xl border border-slate-400 bg-slate-900 p-6">
  ...
  <ul class="space-y-3">
    {#each report.audit_notes as note}
      <li class="border-l-2 border-slate-400 pl-4 text-sm text-slate-400 leading-relaxed">
        {note}
      </li>
    {/each}
  </ul>
